...
```bash
curl http://14.227.143.142:8084/api/data
...
```bash
curl -H "Authorization: Bearer <user_token>" http://14.227.143.142:8084/api/data
# Expected: HTTP 403 Forbidden ✅
```

### Test 4: DATA_MANAGER role truy cập /api/data
```bash
curl -H "Authorization: Bearer <manager_token>" http://14.227.143.142:8084/api/data
# Expected: HTTP 200 OK ✅
```

### Test 5: Direct MySQL API access
```bash
curl http://14.227.143.142:3004/api/mysql?source=mekong
